...
def file_controller(
    ...
    try:
        if action == "list":
            result = list_files(path, show_hidden=bool(parameters.get("show_hidden", False)))

        elif action == "create_file":
            if not name:
                name = f"cristine_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            full = _full_path(path, name)
            result = create_file(full, content=content)

        elif action == "create_folder":
            if not name:
                name = f"Cristine Folder {datetime.now().strftime('%Y-%m-%d')}"
            full = _full_path(path, name)
