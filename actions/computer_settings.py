...
# Cristine — Computer Settings & UI Controls
#
...
def _win_endpoint_volume():
    """Best-effort Windows master volume controller (no UI automation)."""
...
def volume_up():
    if _OS == "Windows":
        try:
            ...
            vol.SetMasterVolumeLevelScalar(min(1.0, cur + 0.10), None)
            return
        except Exception as e:
            print(f"[Settings] âš ï¸ Volume up failed (pycaw): {e}")
            if _PYAUTOGUI:
                for _ in range(5):
...
def volume_down():
    if _OS == "Windows":
        try:
            vol = _win_endpoint_volume()
            cur = float(vol.GetMasterVolumeLevelScalar())
            vol.SetMasterVolumeLevelScalar(max(0.0, cur - 0.10), None)
            return
        except Exception as e:
            print(f"[Settings] âš ï¸ Volume down failed (pycaw): {e}")
            if _PYAUTOGUI:
                for _ in range(5):
...
def volume_mute():
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] âš ï¸ Mute failed (pycaw): {e}")
            if _PYAUTOGUI:
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            vol = _win_endpoint_volume()
            vol.SetMasterVolumeLevelScalar(float(value) / 100.0, None)
...
def take_screenshot():
    # Background-safe screenshot: capture and save without opening OS snipping UI.
    try:
        from mss import mss  # type: ignore
        from PIL import Image  # type: ignore

        out_path = Path.home() / "Desktop" / f"cristine_screenshot_{int(time.time())}.png"
        with mss() as sct:
            mon = sct.monitors[0]  # all monitors
            img = sct.grab(mon)
            im = Image.frombytes("RGB", (img.width, img.height), img.rgb)
            im.save(str(out_path))
        return f"Screenshot saved to Desktop: {out_path}"
    except Exception as e:
        # Fallback to OS hotkeys if available.
        if _PYAUTOGUI:
            try:
                if _OS == "Windows":
                ...
                return "Opened the screenshot tool."
            except Exception:
                pass
...
def open_system_settings():
    if _OS == "Windows":
        try:
            subprocess.Popen(["explorer.exe", "ms-settings:"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception:
            if _PYAUTOGUI:
...
def computer_settings(
        ...
        if not allow_ui:
            if _PYPERCLIP:
                try:
                    ...
                    pass
            return "On-screen automation is disabled. Enable it in Preferences -> Advanced to type into other apps."
        if not _PYAUTOGUI:
    ...
    if action == "press_key":
        ...
        if not allow_ui:
            return "On-screen automation is disabled. Enable it in Preferences -> Advanced to press keys in other apps."
        if not _PYAUTOGUI:
            return "pyautogui is not installed. Run: pip install pyautogui"
        press_key(key)
    ...
    if action in ("reload_n", "refresh_n", "reload_page_n"):
        try:
            n = int(value or 1)
            if not allow_ui:
                return "On-screen automation is disabled. Enable it in Preferences -> Advanced to control the active app."
            if not _PYAUTOGUI:
                return "pyautogui is not installed. Run: pip install pyautogui"
            reload_page_n(n)
    ...
    if not allow_ui and func not in safe_funcs:
        return "On-screen automation is disabled. Enable it in Preferences -> Advanced to control the active app."

    if func not in safe_funcs and not _PYAUTOGUI:
        return "pyautogui is not installed. Run: pip install pyautogui"
