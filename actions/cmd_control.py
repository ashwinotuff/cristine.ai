...
from system.automation_policy import prefer_visible_terminal
...
def cmd_control(
    ...
    visible = (parameters or {}).get("visible", None)
    timeout = (parameters or {}).get("timeout", None)
    try:
        timeout = int(timeout) if timeout is not None else 20
    except Exception:
        timeout = 20
    if visible is None:
        visible = prefer_visible_terminal(player)
