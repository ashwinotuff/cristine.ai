...
from system.automation_policy import prefer_headless_browser
...
class _BrowserThread:
    ...
    def __init__(self):
        ...
        self._context    = None
        self._page       = None
        # Background-safe default: headless automation (no visible browser window).
        self._headless   = True
    ...
    def set_headless(self, headless: bool) -> None:
        ...
        if headless == bool(getattr(self, "_headless", True)):
            return
        self._headless = headless
        # If a browser is already running, close it so we relaunch in the new mode.
        try:
            if self._loop and (self._browser is not None or self._context is not None or self._page is not None):
                fut = asyncio.run_coroutine_threadsafe(self._close_browser_only(), self._loop)
                fut.result(timeout=10)
        except Exception:
            pass

    async def _close_browser_only(self):
        # Keep Playwright running; only close the browser/context/page.
        try:
            if self._context:
    ...
    async def _launch(self):
        ...
        # Headless runs use Playwright's bundled Chromium for reliability and true background execution.
        engine_name, exe_path, channel = "chromium", None, None
        ...
        if engine_name == "chromium" and not headless:
        ...
        try:
            ...
            fb_kwargs = {"headless": headless}
            if not headless:
                fb_kwargs["args"] = ["--start-maximized"]
            self._browser = await self._playwright.chromium.launch(**fb_kwargs)
    ...
    async def _go_to(self, url: str, timeout_ms: int = 15000) -> str:
    ...
    async def _search(self, query: str, engine: str = "google", timeout_ms: int = 15000) -> str:
        engines = {
        ...
        return await self._go_to(url, timeout_ms=timeout_ms)

    async def _click(self, selector=None, text=None, timeout_ms: int = 8000) -> str:
        page = await self._get_page()
        try:
            if text:
                await page.get_by_text(text, exact=False).first.click(timeout=int(timeout_ms))
                return f"Clicked: '{text}'"
            elif selector:
                await page.click(selector, timeout=int(timeout_ms))
    ...
    async def _type(self, selector=None, text: str = "", clear_first: bool = True, timeout_ms: int = 8000) -> str:
        page = await self._get_page()
        try:
            element = page.locator(selector).first if selector else page.locator(":focus")
            if clear_first:
                await element.clear()
            await element.type(text, delay=50, timeout=int(timeout_ms))
            return "Text typed."
        except Exception as e:
    ...
    async def _smart_click(self, description: str, timeout_ms: int = 5000) -> str:
        page       = await self._get_page()
        ...
        for role, keywords in role_hints.items():
            if any(k in desc_lower for k in keywords):
                try:
                    await page.get_by_role(role).first.click(timeout=int(timeout_ms))
                    return f"Clicked ({role}): '{description}'"
                except Exception:
                    pass

        try:
            await page.get_by_text(description, exact=False).first.click(timeout=int(timeout_ms))
            return f"Clicked (text): '{description}'"
        except Exception:
            pass

        try:
            await page.get_by_placeholder(description, exact=False).first.click(timeout=int(timeout_ms))
    ...
    async def _smart_type(self, description: str, text: str, timeout_ms: int = 8000) -> str:
        page = await self._get_page()
        ...
        for method, locator in [
            ...
            try:
                el = locator.first
                await el.clear()
                await el.type(text, delay=50, timeout=int(timeout_ms))
...
def browser_control(
    ...
    parameters:
        ...
        timeout_ms  : Playwright action timeout in milliseconds (optional)
        result_timeout_s : How long to wait for action completion (optional, seconds)
    """
    _ensure_started()

    params = parameters or {}

    headless = params.get("headless", None)
    if headless is None:
        headless = prefer_headless_browser(player)
    _bt.set_headless(bool(headless))

    action = params.get("action", "").lower().strip()
    result = "Unknown action."

    timeout_ms = params.get("timeout_ms", None)
    try:
        timeout_ms = int(timeout_ms) if timeout_ms is not None else None
    except Exception:
        timeout_ms = None

    result_timeout_s = params.get("result_timeout_s", None)
    if result_timeout_s is None:
        result_timeout_s = params.get("result_timeout", None)
    try:
        result_timeout_s = int(result_timeout_s) if result_timeout_s is not None else 30
    except Exception:
        result_timeout_s = 30
    if result_timeout_s < 5:
        result_timeout_s = 5
    if result_timeout_s > 300:
        result_timeout_s = 300

    try:
        if action == "go_to":
            tms = timeout_ms if timeout_ms is not None else 15000
            result = _bt.run(_bt._go_to(params.get("url", ""), timeout_ms=int(tms)), timeout=result_timeout_s)
            ...
            tms = timeout_ms if timeout_ms is not None else 8000
            result = _bt.run(
                _bt._click(
            ...
            result = _bt.run(_bt._fill_form(params.get("fields", {})), timeout=result_timeout_s)

        elif action == "smart_click":
            tms = timeout_ms if timeout_ms is not None else 5000
            result = _bt.run(_bt._smart_click(params.get("description", ""), timeout_ms=int(tms)), timeout=result_timeout_s)
            ...
            result = _bt.run(_bt._get_text(), timeout=result_timeout_s)

        elif action == "press":
            result = _bt.run(_bt._press(params.get("key", "Enter")), timeout=result_timeout_s)

        elif action == "close":
            result = _bt.run(_bt._close_browser(), timeout=result_timeout_s)
