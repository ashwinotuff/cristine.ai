...
# Cristine â€” YouTube Controller
#
# Features:
#   - play      : Search and play a YouTube video (original feature, preserved)
#   - summarize : Get transcript â†’ Gemini summary â†’ speak + optional Notepad
...
def find_video_thumbnails() -> list[tuple[int, int]]:
    ...
    try:
        ...
        print(f"[YouTube] âš ï¸ Thumbnail detection failed: {e}")
        return []
...
def _search_youtube_url(query: str) -> str | None:
    """
    Background-safe YouTube URL resolver.
    Uses DuckDuckGo search (no UI automation) to find a watchable video URL.
    """
    q = (query or "").strip()
    if not q:
        return None
    ...
    try:
        ...
        search_q = f"site:youtube.com/watch {q}"
        with DDGS() as ddgs:
...
def _get_transcript(video_id: str) -> str | None:
    ...
    if not _TRANSCRIPT_OK:
        print("[YouTube] âš ï¸ youtube-transcript-api not installed.")
        return None
    ...
    try:
        ...
        print(f"[YouTube] ðŸ“ Transcript: {len(text)} chars")
        return text
...
def _save_to_notepad(content: str, video_url: str) -> str:
    ...
    print(f"[YouTube] ðŸ’¾ Summary saved: {filepath}")
...
def _scrape_trending(region: str = "TR", max_results: int = 8) -> list[dict]:
    ...
    url = f"https://www.youtube.com/feed/trending?gl={region.upper()}&hl=en&persist_gl=1&persist_hl=1"
    try:
        r    = requests.get(url, headers=HEADERS, timeout=12)
        html = r.text

        def _extract_yt_initial_data(page_html: str) -> dict | None:
            # Locate the ytInitialData assignment and brace-match its JSON.
            markers = [
                "var ytInitialData = ",
                "ytInitialData = ",
            ...
            i = idx + len(mark)
            while i < len(page_html) and page_html[i] in " \t\r\n":
                i += 1
        ...
        def _iter_video_renderers(obj):
            if isinstance(obj, dict):
                ...
                for it in obj:
                    yield from _iter_video_renderers(it)
        ...
        results = []
        seen_ids = set()
        initial = _extract_yt_initial_data(html)
        if isinstance(initial, dict):
            for vr in _iter_video_renderers(initial):
                vid = str(vr.get("videoId") or "").strip()
                if not vid or vid in seen_ids:
                    continue
                title = _get_text(vr.get("title"))
                if not title or len(title) < 3:
                    continue
                channel = _get_text(vr.get("ownerText")) or _get_text(vr.get("longBylineText")) or "Unknown"
                tlo = title.lower().strip()
                if tlo.startswith(("try searching", "keyboard shortcuts", "playback", "general", "subtitles", "spherical")):
                    continue
                seen_ids.add(vid)
                results.append(
                    {
        ...
        for i, title in enumerate(titles):
            if title in seen or len(title) < 5:
                continue
            if title.lower().startswith(("try searching", "keyboard shortcuts", "playback", "general", "subtitles", "spherical")):
                continue
            seen.add(title)
...
def _handle_play(parameters: dict, player) -> str:
    ...
    # Background-safe: resolve a direct video URL and open it (no clicking/typing).
    url = parameters.get("url", "").strip()
    if not url:
        url = _search_youtube_url(query) or ""
    if not url:
        return f"Could not find a playable YouTube link for: {query}"
    ...
    if len(thumbnails) >= 2:
        x, y = thumbnails[1]
        print(f"[YouTube] ðŸŽ¯ Clicking 2nd thumbnail at ({x}, {y})")
        pyautogui.click(x, y)
        return f"Playing YouTube video for: {query}"

    elif len(thumbnails) == 1:
        x, y = thumbnails[0]
        print(f"[YouTube] âš ï¸ One thumbnail found, clicking at ({x}, {y})")
        pyautogui.click(x, y)
        return f"Playing YouTube video for: {query}"

    else:
        print("[YouTube] âš ï¸ No thumbnails found, using fallback position")
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    if not url:
        ...
        if q:
            url = _search_youtube_url(q) or ""
    if not url:
        return "Please provide a YouTube URL or a search query to summarize, sir."
...
def _handle_get_info(parameters: dict, player, speak) -> str:
    ...
    if not url or not _is_valid_youtube_url(url):
        return "Please provide a valid YouTube URL (or a search query), sir."
...
def _handle_trending(parameters: dict, player, speak) -> str:
    ...
    for item in trending:
        ...
        if url:
            lines.append(f"{item['rank']}. {title} - {channel} | {url}")
        else:
            lines.append(f"{item['rank']}. {title} - {channel}")
    result = "\n".join(lines)
...
def youtube_video(
    ...
    Actions:
        ...
        summarize : Summarize a video via transcript + Gemini
                    parameters: save(bool, optional) â€” save summary to Notepad
        get_info  : Get video metadata (title, channel, views, duration)
                    parameters: url(str, optional â€” dialog if omitted)
        trending  : Show trending videos
                    parameters: region(str, default "TR") â€” ISO country code
