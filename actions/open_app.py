# actions/open_app.py
# Cristine — Cross-Platform App Launcher
...
def _launch_windows(app_name: str) -> bool:
    # Background-safe Windows launcher (no keyboard/mouse automation).
    name = (app_name or "").strip().strip('"').strip()
    if not name:
        return False

    # 1) If it's a direct path, run it.
    try:
        p = Path(name).expanduser()
        if p.exists() and (p.is_file() or p.suffix.lower() == ".exe"):
    ...
    # 2) If it's a URL or URI, use ShellExecute via cmd start.
    try:
        if re.match(r"^(https?://|www\\.)", name, flags=re.IGNORECASE) or name.endswith(":"):
    ...
    # 3) If a binary is on PATH, launch it directly.
    try:
        bin_path = shutil.which(name) or shutil.which(name + ".exe")
        if bin_path:
    ...
    # 4) Try PowerShell Start-Process (covers most Win32 apps + registered app paths).
    try:
        ps = (
            "try { Start-Process -FilePath $args[0] -ErrorAction Stop | Out-Null; exit 0 } "
            "catch { exit 1 }"
        )
        r = subprocess.run(
            ["powershell", "-NoProfile", "-Command", ps, name],
            capture_output=True,
            text=True,
            timeout=8,
        )
    ...
    # 5) Try launching Start Menu apps (UWP/MS Store) by AppID.
    try:
        ps = (
            "$name = $args[0]; "
    ...
    # 6) Fallback: let Windows resolve it via ShellExecute.
    try:
        subprocess.Popen(["cmd", "/c", "start", "", name], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(0.4)
        return True
