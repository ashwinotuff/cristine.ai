...
# Cristine — Flight Finder
#
# Searches for flights using Google Flights via browser_control.
# Results are spoken by CRISTINE. Optionally saved to Notepad or opened in browser.
#
# Flow:
...
def _format_notepad(
    ...
    lines = [
        "CRISTINE — Flight Search Results",
        "─" * 50,
...
def flight_finder(
    ...
    # Friendly aliases (older prompts/routines).
    if not origin:
        origin = str(params.get("from_city", "") or params.get("from", "") or "").strip()
    if not destination:
        destination = str(params.get("to_city", "") or params.get("to", "") or "").strip()
    if not date_raw:
        date_raw = str(params.get("depart_date", "") or params.get("departure_date", "") or "").strip()
    return_raw  = params.get("return_date", "").strip()
    pax_val = params.get("passengers", None)
    if pax_val is None:
        pax_val = params.get("adults", 1)
    passengers  = int(pax_val or 1)
