...
SYSTEM_PROMPT = (
    ...
    "Be concise, smart, and helpful like a sharp and efficient AI assistant. "
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript_buf and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Cristine: {full}")
    ...
    async def _play_loop(self):
        ...
        try:
            while True:
                ...
                # Start speaking animation on UI
                if self._player: self._player.root.after(0, self._player.start_speaking)
                
                await asyncio.to_thread(stream.write, chunk)
                
                # Stop speaking if no more audio chunks are queued
                if self._audio_in.empty() and self._player:
                    self._player.root.after(100, self._player.stop_speaking)
        finally:
            stream.close()
